...
contract FeeReimbursement is SafeAccessControlEnumerable {
    ...
    function setDepositHook(IDepositHook _newDepositHook) external onlyRole(SET_DEPOSIT_HOOK_ROLE){
        depositHook = _newDepositHook;
        emit DepositHookChange(_newDepositHook);
    }

    function setPPOToken(IERC20 _newPPOToken) external onlyRole(SET_PPO_TOKEN_ROLE) {
